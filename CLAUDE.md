...
- **Whisper anti-loop is two-layer (ADR-006).** `WhisperService` passes `-mc 0` (`--max-context 0`) to `whisper-cli` so 30-s windows carry zero context tokens forward — without this, hallucinations in one window are fed back as prompt context and self-reinforce until end of audio (the "last sentence repeated 100+ times" failure mode). Do **not** use the older `-nc` / `--no-context` spelling: upstream whisper.cpp removed those flags years ago and modern whisper-cli rejects them with `error: unknown argument: -nc` while exiting **0** — so the failure surfaces as the cryptic generic "no JSON output" path instead of as an arg error. The exact arg list lives in `buildWhisperArgs()` and is locked down by `WhisperService.test.ts` (unit test on the flag set + a skip-if-missing integration test that runs the local binary and asserts it doesn't print "unknown argument"). Defense in depth: `whisper-quality.ts` computes a repetition ratio over the post-processed segments and persists a `quality_flag` on the session — `> 0.3` → `repetition_warning`, `> 0.7` → `repetition_critical`. **Both severities are non-blocking.** The pipeline always runs to completion (diarization → anonymization → summarization) regardless of the flag; the renderer surfaces a `QualityWarningBanner` so the user can spot bad output and file a bug report against the actual broken transcript. There is intentionally no "retry transcription" flow — a deterministic re-run on the same pipeline would produce the same loop.
