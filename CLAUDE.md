...
- **Whisper anti-loop is two-layer (ADR-006).** `WhisperService` passes `-nc` (`--no-context`) to `whisper-cli` so 30-s windows do not condition on the previous window's output — without this, hallucinations self-reinforce until end of audio (the "last sentence repeated 100+ times" failure mode). Defense in depth: `whisper-quality.ts` computes a repetition ratio over the post-processed segments. `> 0.7` throws `QualityRejectionError` → terminal `transcription_quality_failed` state; `> 0.3` persists `quality_flag='repetition_warning'` and lets the pipeline continue with a soft banner. Thresholds are empirical, tune in `whisper-quality.ts`. Bump `TRANSCRIPTION_PIPELINE_VERSION` whenever the whisper invocation or post-processing changes in a way that could yield a different result for the same audio — the renderer gates the manual "Erneut transkribieren" button on `session.transcriptionPipelineVersion < TRANSCRIPTION_PIPELINE_VERSION`, so old failed sessions become re-eligible automatically when the pipeline improves.
